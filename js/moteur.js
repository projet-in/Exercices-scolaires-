...
function vueMot(c, fil, mod){
  aideMot = (mod.langue||'fr')==='fr' && mod.ref==='fr1';
  c.innerHTML = fil + `
...
function questionMot(){
  ...
  const decoys = [];
  const nbDecoys = modActuel.ref==='fr1' ? 0 : 3;
  let garde = 0;
...
function autreTexteAct(){
  speechSynthesis.cancel();
  const pool = LECTURES[modActuel.ref];
  let i = Math.floor(Math.random()*pool.length);
  if(pool.length>1 && i===dernierIdxLecture) i = (i+1)%pool.length;
  dernierIdxLecture = i;
  lectureAct = pool[i];
  indexQLect = 0;
  texteRevele = false;
...
function afficherTexteAct(){
  ...
  if(modeAudition && !texteRevele){
    ...
    el.textContent = lectureAct.t;
    el.classList.remove('texte-audition');
    btn.style.display = 'none';
  }
}
function revelerTexteAct(){ texteRevele = true; afficherTexteAct(); }
function lireTexteAct(){ if(lectureAct) lireVoix(lectureAct.t, modActuel.langue||'fr'); }
function afficherQuestionLect(){
  verrou = false;
  const qs = lectureAct.q;
